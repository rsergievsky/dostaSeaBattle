...
module.exports = {
  ...
  checkPlayer: async function(user_id) {
    ...
    getLikes: for (let i = 0; i < 10; i++) {
      const {response} = JSON.parse(await rp.get(`https://api.vk.com/method/likes.getList?type=post&owner_id=${-env.groupID}&item_id=${env.postID}&count=1&offset=${i*1}&access_token=${cfg.tokens.users[env.tokenIndex]}&v=5.92`));
